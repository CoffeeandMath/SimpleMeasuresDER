...
for i = 1:(Nn-2)
    m(i).BMod = zeros(3,1);
    %m(i).BMod(1) = (w^3 * h/12)*(10^modsc)/Nn; m(i).BMod(2) = (h^3 * w/12)*(10^modsc)/Nn;  m(i).BMod(3) = (h^3 * w/(6*(1+nu)))*(10^modsc)/Nn;%m(i).BMod(3) = m(i).BMod(1) + m(i).BMod(2);
...
while i < Nk
    ...
    plot(phivals(abs(minEval)<10^-3)*180/pi,minEval(abs(minEval)<10^-4)/dS,'.')
    drawnow
end
figure()
