...
for i = 1:(Nn-2)
    m(i).BMod = 2*10^1*ones(3,1)/dS; m(i).BMod(3) = 100*m(i).BMod(1);
    m(i).element = e(i);
end
...
for i = 1:Nk
    ...
    writevtkline(['data/data.' num2str(i) '.vtk'],xv,yv,zv)
    toc
    r.setd2D;
end
%x(floor(end/2)).FreeNode = false; r.calcFreeNodeMask;
Fz = zeros(3,Nk);
Xz = zeros(3,Nk);
for i = 1:Nk
    i+Nk
    x(end).Fnode = multl(i)*1200*Normalize([0;1;1]);
    tic
    s.Solve
    s.LastNumIter
    toc
    ...
    writevtkline(['data/data.' num2str(Nk+i) '.vtk'],xv,yv,zv)
