...
r.Update(2);
r.calcFreeNodeMask;
Constraints = cell(max(size(fc)),1);
for i = 1:max(size(fc))
    Constraints{i} = fc(i);
end
...
for i = 1:Nk
    ...
    tic
    for k = 1:(Nn-2)
        m(k).kappa0 = 0*multl(i)*[0;5;5]*dS;
    end
   x(end).Fnode = (400)*multl(i)*[-1;0;0];
    ...
    writevtkline(['data4/data4.' num2str(i) '.vtk'],xv,yv,zv)
    toc
    r.setd2D;
end
...
for i = 1:Nk
   ...
   x(end).Fnode = (400)*(1-multl(i))*[-1;0;0];
    ...
    Fend(i) = x(end).Fnode(1);
    writevtkline(['data4/data4.' num2str(Nk+i) '.vtk'],xv,yv,zv)
    toc
    r.setd2D;
end

figure()
plot(-Fend/(dS*m(1).BMod(1)),squeeze(Xall(1,end,:)),'.')
hold all
%plot(-Fend/(dS*m(1).BMod(1)),squeeze(Xall(2,end,:)),'.')
plot(-Fend/(dS*m(1).BMod(1)),squeeze(Xall(3,end,:)),'.')
%legend('x','y','z')