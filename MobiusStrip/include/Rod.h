...
public:
	...
	void setPConstInP(std::vector<Constraint*>*);
	std::vector<Constraint*>* getPConstP();
	std::vector<Constraint*>* getPConstInP();
	void SetNodeViscosity(double);
	void SetXtoXold();
	int getNc();
	int getNcIn();
	void SetNumThreads(int);
	void SetActiveIndices(std::vector<int>&);
	void Setdt(double);
protected:
...
private:
	...
	bool NodeViscosityBool;

	double NodeVisc;

	double Evisc;
	Eigen::VectorXd dEvisc;
	std::vector<T> ddEvisctrips;
	std::vector<Eigen::Vector3d> NodePen;
