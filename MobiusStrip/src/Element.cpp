...
void Element::calcdkappadx() {
	for (unsigned int i = 0; i < 3; i++) {
		Quat<double> Bimquat;
		Bimquat.Set(0.0, (Frame1P->getB())(0, i), (Frame1P->getB())(1, i),
				(Frame1P->getB())(2, i));
		//Bimquat.Set(0.0, Bim(0,i),Bim(1,i),Bim(2,i));
		Quat<double> temp = 2.0 * (Frame1P->getd()).conj() * Bimquat
				* (Frame2P->getd());
	...
	for (unsigned int i = 0; i < 3; i++) {
		Quat<double> Biquat;
		Biquat.Set(0.0, (Frame2P->getB())(0, i), (Frame2P->getB())(1, i),
				(Frame2P->getB())(2, i));
		//Biquat.Set(0.0, Bi(0,i),Bi(1,i),Bi(2,i));
		Quat<double> temp = 2.0 * (Frame1P->getd()).conj() * Biquat
				* (Frame2P->getd());
...
void Element::calcddkappadxdphi() {
	for (unsigned int i = 0; i < 3; i++) {
		...
		Biquat.Set(0.0, (Frame2P->getB())(0, i), (Frame2P->getB())(1, i),
				(Frame2P->getB())(2, i));
		//Biquat.Set(0.0, Bi(0,i),Bi(1,i),Bi(2,i));
		Quat<double> dqndphinmdxnm = -1.0 * qe3 * (Frame1P->getd()).conj()
				* Bimquat * (Frame2P->getd());
		Quat<double> dqndphinmdxnp = -1.0 * qe3 * (Frame1P->getd()).conj()
				* Biquat * (Frame2P->getd());
		Quat<double> dqndphindxnm = (Frame1P->getd()).conj() * Bimquat
				* (Frame2P->getd()) * qe3;
		Quat<double> dqndphindxnp = (Frame1P->getd()).conj() * Biquat
				* (Frame2P->getd()) * qe3;
		for (unsigned int j = 0; j < 3; j++) {
			ddkappadxdphi(j, i, 0, 0) = dqndphinmdxnm.Getind(j + 1);
			ddkappadxdphi(j, i, 2, 0) = dqndphinmdxnp.Getind(j + 1);
			ddkappadxdphi(j, i, 1, 0) = -(1.0) * ddkappadxdphi(j, i, 2, 0)
					- ddkappadxdphi(j, i, 0, 0);

			ddkappadxdphi(j, i, 0, 1) = dqndphindxnm.Getind(j + 1);
			ddkappadxdphi(j, i, 2, 1) = dqndphindxnp.Getind(j + 1);
			ddkappadxdphi(j, i, 1, 1) = -(1.0) * ddkappadxdphi(j, i, 0, 1)
					- ddkappadxdphi(j, i, 2, 1);
		}
...
void Element::calcddkappadxdx() {
	...
	for (unsigned int n = 0; n < 3; n++) {
		...
		Quat<double> Biquatn;
		Bimquatn.Set(0.0, (Frame1P->getB())(0, n), (Frame1P->getB())(1, n),
				(Frame1P->getB())(2, n));
		Biquatn.Set(0.0, (Frame2P->getB())(0, n), (Frame2P->getB())(1, n),
				(Frame2P->getB())(2, n));
		for (unsigned int q = 0; q < 3; q++) {
			Quat<double> Bimquatq;
			Quat<double> Biquatq;
			Bimquatq.Set(0.0, (Frame1P->getB())(0, q), (Frame1P->getB())(1, q),
					(Frame1P->getB())(2, q));
			Biquatq.Set(0.0, (Frame2P->getB())(0, q), (Frame2P->getB())(1, q),
					(Frame2P->getB())(2, q));

			Quat<double> DBimnq;
			DBimnq.Set(0.0, (Frame1P->getDB())(0, n, q),
					(Frame1P->getDB())(1, n, q), (Frame1P->getDB())(2, n, q));

			Quat<double> DBinq;
			DBinq.Set(0.0, (Frame2P->getDB())(0, n, q),
					(Frame2P->getDB())(1, n, q), (Frame2P->getDB())(2, n, q));
	...
	for (unsigned int i = 0; i < 3; i++) {
		for (unsigned int j = 0; j < 3; j++) {
			for (unsigned int k = 0; k < 3; k++) {
				ddkappadxdx(i, j, k, 0, 1) = -1.0 * ddkappadxdx(i, j, k, 0, 2)
						- ddkappadxdx(i, j, k, 0, 0);
			}
		}
	}
	...
	for (unsigned int i = 0; i < 3; i++) {
		for (unsigned int j = 0; j < 3; j++) {
			for (unsigned int k = 0; k < 3; k++) {
				ddkappadxdx(i, j, k, 1, 2) = -1.0 * ddkappadxdx(i, j, k, 0, 2)
						- ddkappadxdx(i, j, k, 2, 2);
			}
		}
	}
	for (unsigned int i = 0; i < 3; i++) {
	...
	for (unsigned int i = 0; i < 3; i++) {
		for (unsigned int j = 0; j < 3; j++) {
			for (unsigned int k = 0; k < 3; k++) {
				ddkappadxdx(i, j, k, 1, 1) = -1.0 * ddkappadxdx(i, j, k, 1, 2)
						- ddkappadxdx(i, j, k, 1, 0);
			}
		}
	}
...
Eigen::Vector3d Element::getElemNorm() {
	Quat<double> e2quat;
	e2quat.Set(0.0, 0.0, 1.0, 0.0);
	Eigen::Vector3d nunsc =
			(Frame1P->getd() * e2quat * Frame1P->getd().conj()).Getv();
	nunsc += (Frame2P->getd() * e2quat * Frame2P->getd().conj()).Getv();
	nunsc.normalize();
