...
void Frame::Update(int Order) {
	...
	if (Order > 0) {
		B = (0.5 / eps)
				* ((1.0 / (1.0 + T.dot(t)))
						* (T * (T.cross(t).transpose())
								- T.cross(t) * (t.transpose() + T.transpose()))
						+ skw(T));
	...
	if (Order > 1) {
		...
		Eigen::Vector3d v;
		v = -1.0 * (0.5 * pow(epsinv, 2.0)) * (1.0 / pow(f2i, 2.0))
				* (T - t * (f2i - 1));
		...
		for (unsigned int i = 0; i < 3; i++) {
			for (unsigned int j = 0; j < 3; j++) {
				for (unsigned int k = 0; k < 3; k++) {
					dBidti(i, j, k) = sc
							* (T[i] * M1(j, k) - M1(i, k) * (t[j] + T[j])
									- Ticti[i] * tiprojeps(j, k));
				}
			}
		}
...
Eigen::Vector3d Frame::getd1() {
	Quat<double> e1quat;
	e1quat.Set(0.0, 1.0, 0.0, 0.0);
	Quat<double> d1quat = d * e1quat * d.conj();
	return d1quat.Getv();
}
Eigen::Vector3d Frame::getd2() {
	Quat<double> e2quat;
	e2quat.Set(0.0, 0.0, 1.0, 0.0);
	Quat<double> d2quat = d * e2quat * d.conj();
	return d2quat.Getv();
}
Eigen::Vector3d Frame::getd3() {
	Quat<double> e3quat;
	e3quat.Set(0.0, 0.0, 0.0, 1.0);
	Quat<double> d3quat = d * e3quat * d.conj();
	return d3quat.Getv();
}
